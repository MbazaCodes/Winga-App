...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    // Simple credential check — wire to Supabase in production
    if (email === 'admin@winga.co.tz' && password === 'Admin@Winga2026') {
      // Set session cookie
      document.cookie = 'winga_admin_session=authenticated; path=/; max-age=86400'
      router.push('/')
    } else {
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-8">
          ...
          <p className="text-center text-xs text-gray-400 mt-6">
            Winga Admin Panel v1.0 · Secured Access Only
          </p>
        </div>
      </div>
    </div>
