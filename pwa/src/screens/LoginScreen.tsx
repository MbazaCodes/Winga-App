  ...
  const verifyOtp = async () => {
    ...
    try {
      ...
      if (isPlaceholderName(existing.name)) {
        // Name was never properly set — ask again
    ...
    <div style={{ height: '100dvh', display: 'flex', flexDirection: 'column', background: C.white }}>
      ...
      {step === 'phone' && (
        <div style={{ flex: 1, padding: '24px 24px', overflowY: 'auto' }}>
          ...
          <p style={{ fontFamily: 'Inter', fontSize: 13, color: C.textSec, marginBottom: 24 }}>Ingiza namba yako — tutatuma code ya OTP bure</p>
